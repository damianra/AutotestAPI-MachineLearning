...
api.add_resource(ApiTest, '/test-covid')
...
def autotest():
    ...
    return jsonify({'data': dic, 'disclaimer': ''})
...
def test():
    ...
    df = pd.read_csv('autotest.csv')
    id = int(df.tail(1)['id'] + 1)
    pred = clf.predict([[float(temp), int(zona), int(contacto), int(cansancio), int(perOlf), int(perGus), int(tos),
                         int(difRes), int(pacRiesgo)]])
    dic = {
        'id': id,
